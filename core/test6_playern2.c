...
void do_in3(struct network2_complete *c,int i, int n) {
}



void do_out3(struct network2_complete *c,int i, int n) {
}

void get_receive_buffer(struct network2_complete *c,int i, int n) {}
void get_new_send_buffer(struct network2_complete *c,int i, int n) {}
void get_next_send_buffer_from_queue (struct network2_complete *c,int i, int n) {}
...
int output_screen(network2_complete *c,int result) {
...
for (int i=0;i<12;i++) {
  fprintf(stdout,"%4d 	",c->c1->ports[i]);
  }
fprintf(stdout,"\r\n");
for (int i=0;i<12;i++) {
  fprintf(stdout,"%4d 	",c->c1->sent_to_ports[i]);
  }
...
network2_complete xc;
network2_complete *c = &xc;
volatile network2_complete *cv = (volatile network2_complete *)&xc;


network2_init(c,participant_number,"255.255.255.255",&(argv[1]),
...
int stdin_port = network2_add_standard_input_fd(c,fileno(stdin),&(stdin_in1),NULL,&(stdin_in3),NULL);
...
for (int i=0;i<6;i++) {
  ...
  c->buflen[i]=0;
  c->buflen[i+6]=0;
//  network2_set_buffer(c,i+6,send_buffers[i],0);
  }          
...
//  fprintf(stderr,"poll check\n");
  int result =  network2_poll_check(c);
